...
namespace adventofcode2018
{
    ...
    public class Day03
    {
        static HashSet<(int, int)> MakeSquare(int left, int top, int width, int height)
        {
            return Enumerable.Range(left, width).SelectMany(i => Enumerable.Range(top, height).Select(j => (i, j))).ToHashSet();
        }
        ...
        public static int p1(IEnumerable<string> input) {
            ...
            foreach (var s1 in output.Select((value, i) => new { i, value }))
                foreach (var s2 in output.Select((value, i) => new { i, value }))
                    if (s1.i != s2.i)
                        common.UnionWith(s1.value.Intersect(s2.value));
        ...
        public static int p2(IEnumerable<string> input) {
            ...
            return output.Where(s1 => output.Where(s2 => s2.Key != s1.Key)
